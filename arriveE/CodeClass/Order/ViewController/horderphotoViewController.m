...
@interface horderphotoViewController ()<AMapLocationManagerDelegate,MAMapViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation horderphotoViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"货物拍照";
    self.leftlab.attributedText = [ZLabel attributedTextArray:@[@"13：00\n",@"下单时间"] textColors:@[[UIColor grayColor],[UIColor grayColor]] textfonts:@[H14,H14]];
    self.centerlab.attributedText = [ZLabel attributedTextArray:@[@"¥13.00\n",@"本单收入"] textColors:@[[UIColor redColor],[UIColor grayColor]] textfonts:@[H14,H14]];
    self.rightlab.attributedText = [ZLabel attributedTextArray:@[@"60分钟\n",@"希望送达"] textColors:@[[UIColor grayColor],[UIColor grayColor]] textfonts:@[H14,H14]];
...
- (IBAction)rightbtnaction:(UIButton *)sender
{
    if ([sender.titleLabel.text isEqualToString:@"上传照片"]) {
        [sender setTitle:@"到达送货点" forState:(UIControlStateNormal)];
        [self getupdateimage];
    } else {
        hordercompleViewController *complete = [hordercompleViewController new];
...
- (void)getupdateimage
{
    beizhuCell *cell = [self.tableview cellForRowAtIndexPath:[NSIndexPath indexPathForRow:8 inSection:0]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row >= 0 && indexPath.row < 3){
        ...
        cell.imgbtn.imageView.contentMode = UIViewContentModeScaleAspectFill;
        [cell.imgbtn addTarget:self action:@selector(imagebtnaction:) forControlEvents:(UIControlEventTouchUpInside)];
        return cell;
    }
...
- (void)imagebtnaction:(UIButton *)sender
{
    if (self.imageArray.count == 0) {
        ...
        HJPhotoBrowserController *photoVC = [[HJPhotoBrowserController alloc] initWithPhotos:self.imageArray];
